...
mia_soluzione <- aggiungi_risposta(a = mia_soluzione,
  domanda  = 2,
  risposta = if (exists("death_ita", inherits = FALSE)) death_ita else NULL
)
...
mia_soluzione <- aggiungi_risposta(a = mia_soluzione,
   ...
   risposta = if (any(
     ...
     list(
       numero_righe   = if (exists("numero_righe", inherits = FALSE)) numero_righe else NULL,
       numero_colonne = if (exists("numero_colonne", inherits = FALSE)) numero_colonne else NULL,
       nomi_colonne   = if (exists("nomi_colonne", inherits = FALSE)) nomi_colonne else NULL
     )
   } else {
     NULL
   }
...
death_ita[[1]] <- gsub(" ", "", death_ita[[1]])
causa_del_decesso <- gsub(" ", "", death_ita[[1]])
...
pos_perc_death <- death_ita[death_ita$percentuale > 0, ]
...
mia_soluzione <- aggiungi_risposta(a = mia_soluzione,
  domanda  = 7,
  risposta = if (exists("pos_perc_death", inherits = FALSE)) pos_perc_death else NULL
)
...
death_ita[["mesi"]] <- factor(death_ita[["mesi"]],
  levels = c("0-59", "0-1", "2-59"),
  labels = c("prescolare", "neonato", "fanciullo")
)
...
mia_soluzione <- aggiungi_risposta(a = mia_soluzione,
  domanda  = 9,
  risposta = if (exists("max_p_death_causes", inherits = FALSE)) max_p_death_causes else NULL
)
...
top_fan_2000 <- topdown[topdown$mesi == 'neonato' & topdown$anno == 2000, ]
top_fan_2016 <- topdown[topdown$mesi == 'neonato' & topdown$anno == 2016, ]
...
mia_soluzione <- aggiungi_risposta(a = mia_soluzione,
   ...
   risposta = if (any(
     ...
     list(
       top_fan_2000 = if (exists("top_fan_2000", inherits = FALSE)) top_fan_2000 else NULL,
       top_fan_2016 = if (exists("top_fan_2016", inherits = FALSE)) top_fan_2016 else NULL
     )
   } else{
     NULL
   }
...
only_top_2000 <- death_ita[
  death_ita$causa_del_decesso %in% top_fan_2000[['causa_del_decesso']][1:3],
  ]
...
only_top_2016 <- death_ita[
  death_ita$causa_del_decesso %in% top_fan_2016[['causa_del_decesso']][1:3],
  ]
xyplot(percentuale ~ anno | mesi,
       groups = causa_del_decesso,
       data   = only_top_2016,
