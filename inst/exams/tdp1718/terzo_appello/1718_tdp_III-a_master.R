...
#'
#'   > submit_my_exam("1718_tdp_III-a.R")
#'
...
#'
#' Per uniformità di linguaggio, rinominare le colonne con nome inglese
#' in italiano.
#'
#' In particolare, scrivere nello spazio non commentato qui sotto, il
#' codice utile a modificare il nome della colonna `cause_of_death` in
#' `causa_del_decesso`.
#'
#'<!-- ============================================================= -->


is_a_name_to_change <- colnames(death_ita) == "cause_of_death"

colnames(death_ita)[is_a_name_to_change] <- "causa_del_decesso"
...
#' `causa_del_decesso` in caratteri tutti minuscoli Per esempio,
...
death_ita[["causa_del_decesso"]] <- tolower(
...
#' variabile `causa_del_decesso` del dataframe `death_ita`) ogni
#' occorrenza dello spazio. Per esempio, `diarrhoeal disease` deve
#' diventare `diarrhoealdisease`.
#'
#'<!-- ============================================================= -->


death_ita[["causa_del_decesso"]] <- gsub(
  pattern     = " +",
  replacement = "",
  x = death_ita[["causa_del_decesso"]]
)
...
max_p_death_causes <- tapply(death_ita[["percentuale"]],
...
#' 2000 e ai neonati, ordinati dalla causa più frequente alla meno
...
top_fan_2000_unordered <- death_ita[
  death_ita[["anno"]] == 2000 & death_ita[["mesi"]] == "neonato",
]
top_fan_2000 <- top_fan_2000_unordered[
  order(top_fan_2000_unordered[["percentuale"]], decreasing = TRUE),
]


top_fan_2016_unordered <- death_ita[
  death_ita[["anno"]] == 2016 & death_ita[["mesi"]] == "neonato",
  ]
top_fan_2016 <- top_fan_2016_unordered[
  order(top_fan_2016_unordered[["percentuale"]], decreasing = TRUE),
  ]
...
only_top_2000 <- death_ita[
  death_ita$causa_del_decesso %in%
  top_fan_2000[['causa_del_decesso']][1:3],
]
...
only_top_2016 <- death_ita[
  death_ita$causa_del_decesso %in%
  top_fan_2016[['causa_del_decesso']][1:3],
]
xyplot(percentuale ~ anno | mesi,
  groups   = causa_del_decesso,
  data     = only_top_2016,
