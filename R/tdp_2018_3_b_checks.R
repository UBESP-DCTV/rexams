...
#' @export
tdp_2018_3_b_check_1 <- function(wd = getwd(), file_to_check = "death_ita.csv"){
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_2 <- function(main_data) {
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_3 <- function(data, reference) {
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_4 <- function(main_df) {
  if ('percentuale' %in% colnames(main_df)) {
    message(
      'la colonna `percentuale` sembra essere ancora presente nel dataset.\n'
    )
    return(invisible(-1L))
  } else if (
    !'perc' %in% colnames(main_df)
  ) {
    message(
      'la colonna `perc` non sembra presente nel dataset.\n'
    )
    return(invisible(-1L))
  } else {
    message(
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_5 <- function(main_df, .fun = toupper
) {
  if (is.null(main_df[["cause_of_death"]])) {
    message("L'esercizio non sembra essere corretto...\n")
    message("variabile cause_of_death non presente nel dataset")
    return(invisible(-1L))
  }
  if (all(
    main_df[["cause_of_death"]] ==
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_6 <- function(main_df, new_value = '_') {
  if (is.null(main_df[["cause_of_death"]])) {
    message("L'esercizio non sembra essere corretto...\n")
    message("variabile cause_of_death non presente nel dataset")
    return(invisible(-1L))
  ...
  if (all(
    tolower(main_df[["cause_of_death"]]) ==
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_7 <- function(main_df) {
  if (
    isTRUE(all.equal(
      main_df[["perc"]],
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_8 <- function(main_df,
  ord_one   = 'perc',
  ord_two   = 'anno',
  ord_three = 'mesi'
) {
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_9 <- function(main_table) {
...
#' @return logical (invisibly)
#' @export
tdp_2018_3_b_check_10 <- function(main_data) {
  ...
  try(silent = TRUE, {
    if (!is.data.frame(top_fan_2000)) {
      ...
      top_fan_2000[['cause_of_death']][1:2] ==
        c("PERTUSSIS", "TETANUS")          &&

        top_fan_2000[['perc']][2:3] == c(0, 0) &&

        all(top_fan_2000[['anno']] == 2000)                 &&

        all(top_fan_2000[['mesi']] %in% c('fanciullo'))
    ) {
  ...
  try(silent = TRUE, {
    if (!is.data.frame(top_fan_2016)) {
      ...
      top_fan_2016[['cause_of_death']][1:2] ==
        c("HIV/AIDS", "TETANUS")            &&

        top_fan_2016[['perc']][2:3] == c(0, 0) &&

        all(top_fan_2016[['anno']] == 2016)                 &&

        all(top_fan_2016[['mesi']]  %in% c('fanciullo'))
    ) {
